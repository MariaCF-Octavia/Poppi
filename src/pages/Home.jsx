...
export default function Home({ session }) {
  ...
  async function createRoom(e) {
    ...
    if (!error && data) {
      ...
      setShowCreate(false)
      setRoomName(''); setRoomTopic(''); setRoomCategory('Culture')
      ...
      {showCreate && (
        <div style={s.overlay}>
          <div style={s.modal}>
            ...
            <div style={s.typeRow}>
              {[false, true].map(priv => (
                <div key={String(priv)} style={{...s.typeCard, ...(isPrivate === priv ? s.typeCardActive : {})}} onClick={() => setIsPrivate(priv)}>
            ...
            <form onSubmit={createRoom} style={s.form}>
              ...
              <div style={s.formGroup}>
                ...
                <textarea style={{...s.input, minHeight:'72px', resize:'vertical', lineHeight:1.5}} placeholder="The topic or theme of this room" value={roomTopic} onChange={e => setRoomTopic(e.target.value)} />
              </div>
              <div style={s.formGroup}>
      ...
      <div style={s.list}>
        {noResults && (
          <div style={s.noResults}>
            <div style={s.noResultsTitle}>{search ? `No rooms found for "${search}"` : `No ${activeFilter} rooms yet`}</div>
        ...
        {publicRooms.length > 0 && (
          <>
            ...
            {publicRooms.map(room => {
              ...
              return (
                <div key={room.id} style={{...s.card, opacity: isDeleting ? 0.5 : 1}} onClick={() => joinRoom(room)}>
                  ...
                  <div style={s.cardBody}>
                    ...
                    {preview ? (
                      <div style={s.cardPreview}>
                        ...
                        <span style={s.previewText}>{preview.text.length > 85 ? preview.text.slice(0,85)+'…' : preview.text}</span>
                      </div>
        ...
        {privateRooms.length > 0 && (
          <>
            ...
            {privateRooms.map(room => {
              ...
              const isDeleting = deleting === room.id
              return (
                <div key={room.id} style={{...s.card, borderColor:'rgba(192,0,58,0.08)', opacity: isDeleting ? 0.5 : 1}} onClick={() => joinRoom(room)}>
                  <div style={{...s.cardAv, background:'#1a0010', border:'1px solid rgba(192,0,58,0.2)'}}>
                    <span style={{...s.cardAvText, color:'rgba(245,224,234,0.4)', fontSize:'10px'}}>PVT</span>
                  </div>
                  <div style={s.cardBody}>
                    <div style={s.cardTop}>
                      <div style={s.cardName}>{room.name}</div>
                      <div style={{...s.liveBadge, background:'rgba(255,255,255,0.04)', borderColor:'rgba(255,255,255,0.08)', color:'rgba(245,224,234,0.3)'}}>PRIVATE</div>
                    </div>
                    {room.topic && <div style={s.cardTopic}>{room.topic}</div>}
                    {preview ? (
                      <div style={s.cardPreview}>
...
const s = {
  wrap: { minHeight:'100vh', background:'#070003', color:'#f5e0ea', fontFamily:"'DM Sans','Helvetica Neue',sans-serif", position:'relative' },
  ...
  clearBtn: { background:'none', border:'none', color:'rgba(245,224,234,0.3)', cursor:'pointer', fontSize:'11px', padding:0, fontFamily:'inherit' },
  filterRow: { display:'flex', gap:'8px', padding:'8px 16px 12px', overflowX:'auto', scrollbarWidth:'none', msOverflowStyle:'none' },
  ...
  overlay: { position:'fixed', inset:0, background:'rgba(0,0,0,0.88)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:100, padding:'20px' },
  ...
  card: { display:'flex', alignItems:'flex-start', gap:'12px', padding:'14px 16px', background:'#0e0007', border:'1px solid rgba(192,0,58,0.12)', borderRadius:'16px', marginBottom:'10px', cursor:'pointer', position:'relative', overflow:'hidden', transition:'border-color 0.15s' },
  cardAv: { width:'44px', height:'44px', borderRadius:'12px', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 },
  cardAvText: { fontFamily:"'Space Mono',monospace", fontSize:'12px', fontWeight:'700', color:'rgba(255,255,255,0.85)', letterSpacing:'0.02em' },
  cardBody: { flex:1, minWidth:0 },
  cardTop: { display:'flex', alignItems:'center', justifyContent:'space-between', gap:'8px', marginBottom:'4px' },
  cardName: { fontSize:'15px', fontWeight:'600', color:'#f5e0ea', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' },
  cardRight: { display:'flex', alignItems:'center', gap:'5px', flexShrink:0 },
  hotBadge: { fontSize:'9px', fontFamily:"'Space Mono',monospace", color:'#ff6b35', background:'rgba(255,107,53,0.12)', border:'1px solid rgba(255,107,53,0.25)', padding:'2px 7px', borderRadius:'100px', letterSpacing:'0.06em' },
  catBadge: { fontSize:'9px', fontFamily:"'Space Mono',monospace", color:'rgba(245,224,234,0.35)', background:'rgba(245,224,234,0.04)', border:'1px solid rgba(245,224,234,0.08)', padding:'2px 7px', borderRadius:'100px', letterSpacing:'0.04em' },
  liveBadge: { display:'flex', alignItems:'center', gap:'4px', fontSize:'9px', fontFamily:"'Space Mono',monospace", color:'#4ade80', background:'rgba(74,222,128,0.08)', border:'1px solid rgba(74,222,128,0.18)', padding:'2px 7px', borderRadius:'100px', letterSpacing:'0.06em' },
  liveDot: { width:'5px', height:'5px', borderRadius:'50%', background:'#4ade80', animation:'pulse 2s infinite' },
  cardTopic: { fontSize:'12px', color:'rgba(245,224,234,0.4)', marginBottom:'6px', lineHeight:1.4, overflow:'hidden', display:'-webkit-box', WebkitLineClamp:1, WebkitBoxOrient:'vertical' },
  ...
  deleteBtn: { background:'rgba(245,224,234,0.04)', border:'1px solid rgba(245,224,234,0.06)', color:'rgba(245,224,234,0.25)', width:'26px', height:'26px', borderRadius:'8px', cursor:'pointer', fontSize:'10px', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, fontFamily:'inherit' },
